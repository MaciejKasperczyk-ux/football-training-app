    ...
    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-4">
      ...
      {!editing ? (
        <div className="mt-4 grid gap-1 text-sm text-gray-700">
          ...
          <div>
            Trenerzy: {trainers.length ? trainers.map((id) => {
              const t = allTrainers.find((x) => String(x._id) === id);
              return <span key={id} className="inline-block rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-700 mr-1">{t?.name ?? t?.email ?? id}</span>;
